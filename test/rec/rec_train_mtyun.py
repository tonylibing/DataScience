...
def main(_):
    ...
    # cs = ColumnSummary(feature_matrix.todense())
    # print(cs)
    # with tf.gfile.Open(os.path.join(args.data_dir, "rec_data_train_feature_matrix.npz"), 'wb') as wf:
    #     scipy.sparse.save_npz(wf, feature_matrix)
    print(str(bfp))
    ...
    print("Xgboost Test F1 Score: {0}".format(f1_score(y_test, y_pre)))
    ...
    print("lightgbm Test F1 Score: {0}".format(f1_score(y_test, y_pre)))
