...
def main(_):
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir,rawpath,'user.csv'), 'rb') as gf:
        userfile = pd.read_csv(gf)
    with tf.gfile.FastGFile(os.path.join(data_dir,rawpath,'app_categories.csv'), 'rb') as gf:
        appfile = pd.read_csv(gf)
    with tf.gfile.FastGFile(os.path.join(data_dir, rawpath, 'user_installedapps.csv'), 'rb') as gf:
        installed = pd.read_csv(gf)
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'appInstalledusercount.csv'), 'wb') as gf:
        temp.to_csv(gf, index=False)

    temp = installed.groupby('appCategory')['userID'].count().reset_index()
    temp.columns = ['appCategory', 'appCat_usercount']
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'appCatInstalledusercount.csv'), 'wb') as gf:
        temp.to_csv(gf, index=False)

    # user，edu，age,gender安装app数
    temp = installed.groupby('userID')['appID'].count().reset_index()
    temp.columns = ['userID', 'user_appcount']
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'userInstalledappscount.csv'), 'wb') as gf:
        temp.to_csv(gf, index=False)

    temp = installed.groupby('education')['appID'].count().reset_index()
    temp.columns = ['education', 'edu_appcount']
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'eduuserInstalledappscount.csv'), 'wb') as gf:
        temp.to_csv(gf, index=False)

    temp = installed.groupby('age')['appID'].count().reset_index()
    temp.columns = ['age', 'age_appcount']
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'ageuserInstalledappscount.csv'), 'wb') as gf:
        temp.to_csv(gf, index=False)

    temp = installed.groupby('gender')['appID'].count().reset_index()
    temp.columns = ['gender', 'gender_appcount']
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'genderuserInstalledappscount.csv'), 'wb') as gf:
        temp.to_csv(gf, index=False)

    print('installed over...')

    ##########################################################actions文件提取特征，7天滑窗，统计用户安装的app数，app被安装的用户数
    with tf.gfile.FastGFile(os.path.join(data_dir,rawpath,'user_app_actions.csv'), 'rb') as gf:
        actions = pd.read_csv(gf)
    actions['day'] = actions['installTime'] // 10000
    res = pd.DataFrame()
    temp = actions[['userID', 'day', 'appID']]
    for day in range(28, 32):
        count = temp.groupby(['userID']).apply(
            lambda x: x['appID'][(x['day'] < day).values & (x['day'] > day - 8).values].count()).reset_index(
            name='appcount')
        count['day'] = day
        res = res.append(count, ignore_index=True)

    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'all_user_seven_day_cnt.csv'), 'wb') as gf:
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'all_app_seven_day_cnt.csv'), 'wb') as gf:
        res.to_csv(gf, index=False)

    print('actions over...')

    X_loc_train, X_loc_test = readData(data_dir,m_type='inner', drop=True)
    print('readData over')
    ...
    for feat_1 in ['creativeID', 'positionID', 'userID', 'sitesetID']:
        ...
        for day in range(28, 32):
            ...
            res = res.append(count, ignore_index=True)
        print(feat_1, ' over')
        with tf.gfile.FastGFile(os.path.join(data_dir, temppath,'%s.csv' % feat_1), 'wb') as gf:
            res.to_csv(gf, index=False)
    ...
    for feat_1, feat_2 in [('positionID', 'advertiserID'), ('userID', 'sitesetID'), ('positionID', 'connectionType'),
        ...
        for day in range(28, 32):
            count = temp.groupby([feat_1, feat_2]).apply(
                lambda x: x['label'][(x['day'] < day).values].count()).reset_index(name=feat_1 + '_' + feat_2 + '_all')
            count1 = temp.groupby([feat_1, feat_2]).apply(
                lambda x: x['label'][(x['day'] < day).values].sum()).reset_index(name=feat_1 + '_' + feat_2 + '_1')
            count[feat_1 + '_' + feat_2 + '_1'] = count1[feat_1 + '_' + feat_2 + '_1']
            count.fillna(value=0, inplace=True)
            count['day'] = day
            res = res.append(count, ignore_index=True)
        print(feat_1, feat_2, ' over')
        with tf.gfile.FastGFile(os.path.join(data_dir, temppath,'%s.csv' % (feat_1 + '_' + feat_2)), 'wb') as gf:
            res.to_csv(gf, index=False)
    ...
    for feat_1, feat_2, feat_3 in [('appID', 'connectionType', 'positionID'), ('appID', 'haveBaby', 'gender')]:
        ...
        with tf.gfile.FastGFile(os.path.join(data_dir, temppath, '%s.csv' % (feat_1 + '_' + feat_2 + '_' + feat_3)), 'wb') as gf:
            res.to_csv(gf, index=False)

    ##########################################################比赛官方群里大神分享过的，这里用app平均回流时间做特征，缺失的用app类别的平均回流时间替代
    X_loc_train, X_loc_test = readData(data_dir,m_type='inner', drop=False)
    del X_loc_test
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, temppath, 'app_cov_diffTime.csv'),'wb') as gf:
        grouped.to_csv(gf, index=False)

    grouped = X_loc_train.groupby('appCategory')['cov_diffTime'].mean().reset_index()
    grouped.columns = ['appCategory', 'appCat_cov_diffTime']
    with tf.gfile.FastGFile(os.path.join(data_dir, temppath, 'appCat_cov_diffTime.csv'),'wb') as gf:
        grouped.to_csv(gf, index=False)
...
def readData(data_dir,m_type='inner',drop=True):  ###################使用Trick时，left merge不改变顺序会比inner merge差1个万分点左右？
    with tf.gfile.FastGFile(os.path.join(data_dir,rawpath,'train.csv'), 'rb') as gf:
        X_train = pd.read_csv(gf)

    with tf.gfile.FastGFile(os.path.join(data_dir, rawpath, 'test.csv'), 'rb') as gf:
        X_test = pd.read_csv(gf)
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, rawpath, 'ad.csv'), 'rb') as gf:
        adfile = pd.read_csv(gf)
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, rawpath, 'position.csv'), 'rb') as gf:
        positionfile = pd.read_csv(gf)
...
def doPre(data):
    ...
    data['hour'] = data['clickTime'] % 10000 // 100
    return data
