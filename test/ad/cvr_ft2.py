...
def main(_):
    ...
    X_loc_train, X_loc_test = readData(data_dir,m_type='inner', scope=(28, 30))
    print('readData over')
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir,iapath,'all_app_seven_day_cnt.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['appID', 'day'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['appID', 'day'])

    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'all_user_seven_day_cnt.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['userID', 'day'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['userID', 'day'])
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'userInstalledappscount.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['userID'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['userID'])
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'appInstalledusercount.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'ageuserInstalledappscount.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['age'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['age'])
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'appCatInstalledusercount.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['appCategory'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['appCategory'])
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'eduuserInstalledappscount.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['education'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['education'])
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, 'genderuserInstalledappscount.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)
    ...
    ##########################################################appID平均回流时间特征
    with tf.gfile.FastGFile(os.path.join(data_dir, temppath, 'app_cov_diffTime.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)

    X_loc_train = pd.merge(X_loc_train, temp, how='left', on=['appID'])
    X_loc_test = pd.merge(X_loc_test, temp, how='left', on=['appID'])
    with tf.gfile.FastGFile(os.path.join(data_dir, temppath, 'appCat_cov_diffTime.csv'), 'rb') as gf:
        temp = pd.read_csv(gf)
    ...
    for feat_1 in ['creativeID', 'positionID', 'userID']:
        with tf.gfile.FastGFile(os.path.join(data_dir, temppath,  '%s.csv' % feat_1), 'rb') as gf:
            temp = pd.read_csv(gf)
    ...
    for feat_1, feat_2, feat_3 in [('appID', 'connectionType', 'positionID'), ('appID', 'haveBaby', 'gender')]:
        with tf.gfile.FastGFile(os.path.join(data_dir, temppath, '%s.csv' % (feat_1 + '_' + feat_2 + '_' + feat_3)), 'rb') as gf:
            temp = pd.read_csv(gf)
        bs = BayesianSmoothing(1, 1)
    ...
    for feat_1, feat_2 in [('positionID', 'advertiserID'), ('userID', 'sitesetID'), ('positionID', 'connectionType'),
                           ...
                           ('appID', 'hour'), ('hour', 'age')]:
        with tf.gfile.FastGFile(os.path.join(data_dir, temppath, '%s.csv' % (feat_1 + '_' + feat_2)), 'rb') as gf:
            temp = pd.read_csv(gf)
    ...
    print(X_loc_train.shape)
    print(X_loc_train.columns)
    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, '2_smooth.csv'), 'wb') as gf:
        X_loc_train.to_csv(gf, index=False)

    with tf.gfile.FastGFile(os.path.join(data_dir, iapath, '2_test_smooth.csv'), 'wb') as gf:
        X_loc_test.to_csv(gf, index=False)
...
def readData(data_dir,m_type='inner', scope=(28, 30)):  ###################left merge不改变顺序会比inner merge差1到2个万分点
    with tf.gfile.FastGFile(os.path.join(data_dir, rawpath, 'train.csv'), 'rb') as gf:
        X_train = pd.read_csv(gf)

    pos=(X_train['clickTime']//10000>=scope[0]).values & (X_train['clickTime']//10000<=scope[1]).values
