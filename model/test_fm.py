...
LIBFM_PATH = '/home/tanglek/opensource/libfm/bin/libFM'
...
def fitpredict_libfm(trainX, trainY, testX, classification=True, rank=8, n_iter=100):
    ...
    console_output = get_ipython().getoutput(
        u"$LIBFM_PATH -task $task -method mcmc -train $train_file -test $test_file -iter $n_iter -dim '1,1,$rank' -out output.libfm")
...
for name in ['ml100k, ids', 'ml-1m,ids', 'ml100k', 'ml-1m', 'flight1m', 'flight1m, ext', 'flight10m', 'avazu100k',
             'avazu1m']:
    df = all_results[name]
